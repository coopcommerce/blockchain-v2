import { OffspringFactory } from "./../typechain/OffspringFactory.d";
...
describe("OffspringFactory", () => {
  ...
  async function fixture([wallet, other]: Wallet[]) {
    const tmp = await ethers.getContractFactory("OffspringFactory");
  ...
  async function createPair(
    factory: OffspringFactory,
    tokens: [string, string]
  ) {
    const pairContract = await ethers.getContractFactory("OffspringPair");
    ...
    await expect(factory.createPair(tokens[0], tokens[1])).to.be.reverted; // Offspring: PAIR_EXISTS
    await expect(factory.createPair(tokens[1], tokens[0])).to.be.reverted; // Offspring: PAIR_EXISTS
  ...
  it("setFeeTo", async () => {
    ...
    ).to.be.revertedWith("Offspring: FORBIDDEN");
